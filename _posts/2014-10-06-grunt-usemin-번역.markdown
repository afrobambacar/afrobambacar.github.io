...
> HTML 파일들 (혹은 template/views 디렉토리 안의 HTML 파일들) 안에 설정된 최적화 되지 않은 스크립트, 스타일시트 그리고 기타 참조 파일들을 최적화된 파일로 교체해준다.

**[Maintainer wanted](https://github.com/yeoman/grunt-usemin/issues/313)**

## Getting Started
만약 그런트를 사용해본적이 없다면 그런트의 Getting Started Guide를 확인할 것. 거기에는 gruntfile을 어떻게 생성하고 그런트 플러그인을 어떻게 설치하는지 나와있다. 이 과정에 한번 익숙해진 사용자라면 이 플러그인은 다음의 명령어를 통해 설치한다.
...
`usemin`은 HTML 파일 내부에 정의되어 있는 스크립트, 스타일시트 기타 참조 파일들을 최적화된 버전으로 교체해준다. 이 작업을 하기 위해 `usemin`은 `useminPrepare`와 `usemin`으로 불리는 2개의 빌트인된 태스크를 호출하고 다른 그런트 플러그인들과 함께 동작하면서 최적화시키는 과정을 거치게 된다.

The built-in tasks of `usemin`:
* `useminPrepare`는 여러개의 파일이 있는 특정 블록을 파일의 최적화된 버전을 가리키도록 한줄로 변환시키는 짓을 한다. 



prepares the configuration to transform specific blocks in the scrutinized file into a single line, targeting an optimized version of the files. This is done by generating subtasks called `generated` for every optimization steps handled by the Grunt plugins listed below.
...
* [`concat`](https://github.com/gruntjs/grunt-contrib-concat) 파일 합치기 (일반적으로 JS 혹은 CSS).
* [`uglify`](https://github.com/gruntjs/grunt-contrib-uglify) JS 파일들을 작게 만들어 준다.
* [`cssmin`](https://github.com/gruntjs/grunt-contrib-cssmin) CSS 파일들을 작게 만들어 준다.
* [`filerev`](https://github.com/yeoman/grunt-filerev) 파일의 컨텐트 해쉬로 정적 파일들의 이름을 수정해준다..

**Important**: _위 플러그인들은 수동으로 설치해야 하며 로드해야 한다_.
